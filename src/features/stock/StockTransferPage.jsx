...
function NewTransferModal({ onClose, onCreated, locations, items }) {
  ...
  return (
    <Modal title="สร้างใบย้ายคลังสินค้า" onClose={onClose}>
      ...
      <p className="text-sm font-medium text-stone-700 dark:text-stone-200 mb-2">รายการสินค้าที่ย้าย</p>
      <div className="space-y-2 mb-2">
        {lines.map((line, idx) => (
          <div key={idx} className="flex items-start gap-2">
            ...
            {lines.length > 1 && (
              <button onClick={() => setLines((ls) => ls.filter((_, i) => i !== idx))} className="p-2 text-stone-400 hover:text-rose-500"><Trash2 className="w-4 h-4" /></button>
            )}
          </div>
        ))}
      </div>
      <button onClick={() => setLines((ls) => [...ls, { stockItemId: "", label: "", qty: 1 }])} className="text-sm font-medium text-stone-900 hover:text-stone-900 mb-4">+ เพิ่มสินค้าอีกรายการ</button>

      <p className="text-xs text-stone-400 mb-2">ระบบจะตัด On Hand ออกจากคลังต้นทางทันทีที่สร้างใบย้าย และเพิ่มเข้าคลังปลายทางเมื่อกด "ยืนยันรับของ"</p>
  ...
  return (
    <div>
      <div className="flex items-start justify-between mb-6">
        <div>
          <div className="flex items-center gap-1.5 text-xs text-stone-400 mb-1.5"><span>Stock</span><span>/</span><span className="text-stone-900 font-medium">ย้ายคลังสินค้า</span></div>
          <h1 className="text-2xl font-bold text-stone-900 dark:text-stone-100 tracking-tight">ย้ายคลังสินค้า</h1>
        </div>
      ...
      <Card className="overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-stone-100 dark:divide-stone-700">
            {loading && <tr><td colSpan={7} className="text-center text-stone-400 py-10">กำลังโหลด...</td></tr>}
            {!loading && transfers?.length === 0 && <tr><td colSpan={7} className="text-center text-stone-400 py-10">ยังไม่มีใบย้ายคลัง</td></tr>}
            {transfers?.map((t) => (
              <React.Fragment key={t.id}>
                <tr className="cursor-pointer hover:bg-stone-50/50 dark:hover:bg-stone-700/30" onClick={() => setExpanded(expanded === t.id ? null : t.id)}>
                  <td className="px-4 py-3 text-stone-400">
                    {expanded === t.id ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
                  </td>
                  <td className="px-4 py-3 font-medium text-stone-900">{t.transfer_no}</td>
                  <td className="px-4 py-3 text-stone-600 dark:text-stone-300">{t.from_location?.name}</td>
                  <td className="px-4 py-3 text-stone-600 dark:text-stone-300">{t.to_location?.name}</td>
                  <td className="px-4 py-3 text-right text-stone-600 dark:text-stone-300">{t.items?.length ?? 0} รายการ</td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="inline-flex items-center gap-3">
                      {t.status !== "received" && canManage && (
                        <button onClick={(e) => { e.stopPropagation(); handleReceive(t.id); }} className="text-xs font-medium text-stone-900 hover:underline">ยืนยันรับของ</button>
                      )}
                      {canManage && (
                        <button onClick={(e) => { e.stopPropagation(); handleDelete(t); }} className="text-stone-400 hover:text-rose-500"><Trash2 className="w-4 h-4" /></button>
                      )}
                    </div>
                  </td>
                </tr>
                {expanded === t.id && (
                  <tr>
                    <td colSpan={7} className="px-4 py-3 bg-stone-50/50 dark:bg-stone-900/40">
                      {(t.items || []).length === 0 ? (
                        <p className="text-xs text-stone-400">ใบย้ายรุ่นเก่า (ก่อนอัปเดต) — ไม่มีรายการสินค้าบันทึกไว้ และไม่มีผลกับยอดสต็อก</p>
                      ) : (
                          ...
                          <tbody className="divide-y divide-stone-100 dark:divide-stone-700">
                            {(t.items || []).map((it) => (
                              <tr key={it.id}>
                                ...
                                <td className="py-2 text-stone-500">{it.item?.description || "-"}</td>
                                <td className="py-2 text-right text-stone-600 dark:text-stone-300">{it.qty}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
