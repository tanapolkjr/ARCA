...
export function TextArea(props) {
  return (
    <textarea
      {...props}
      className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-sm text-slate-800 dark:text-slate-100 bg-slate-50 dark:bg-slate-900 focus:bg-white dark:focus:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500"
    />
  );
}
...
export function Select({ children, ...props }) {
  return (
    <div className="relative">
      <select
        {...props}
        className="w-full appearance-none px-3.5 py-2.5 pr-9 rounded-xl border border-slate-200 dark:border-slate-700 text-sm text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-slate-50 dark:bg-slate-900 focus:bg-white dark:focus:bg-slate-800 transition-colors"
      >
        {children}
      </select>
          ...
          <div className="absolute z-10 mt-1.5 w-full bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-lg max-h-56 overflow-auto py-1.5">
            ...
            {!searching && items.map((o) => (
              <button
                ...
                className="w-full text-left px-3.5 py-2 text-sm text-slate-700 dark:text-slate-200 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 hover:text-indigo-700 dark:hover:text-indigo-300 transition-colors"
              >
                {o.label}
              </button>
            ))}
            {onAddNew && (
              <div className="border-t border-slate-100 dark:border-slate-700 mt-1 pt-1">
                <button
                  ...
                  className="w-full flex items-center gap-2 text-left px-3.5 py-2 text-sm font-medium text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 transition-colors"
                >
  ...
  return (
    <button
      ...
      className={`text-left bg-white dark:bg-slate-800 rounded-2xl border shadow-sm p-4 transition-colors ${
        active ? "border-indigo-400 ring-2 ring-indigo-100 dark:ring-indigo-500/20" : "border-slate-200 dark:border-slate-700 hover:border-indigo-200"
      }`}
    >
...
export function Banner({ tone = "error", children }) {
  const tones = {
    ...
    info: "bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-300",
  };
