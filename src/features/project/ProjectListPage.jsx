...
export default function ProjectList() {
  ...
  const statusCards = [
    { key: "all", label: "ทั้งหมด", chip: "bg-stone-400" },
    { key: "New Request", label: "New Request", chip: "bg-stone-400" },
    { key: "Request Accepted", label: "Request Accepted", chip: "bg-stone-800" },
    ...
    { key: "Equipment Shipped", label: "Equipment Shipped", chip: "bg-stone-500" },
  ];
  ...
  return (
    <div>
      <div className="flex items-start justify-between mb-6 flex-wrap gap-3">
        <div>
          <div className="flex items-center gap-1.5 text-xs text-stone-400 mb-1.5">
            <span>E-Service</span><span>/</span><span>Service</span><span>/</span><span className="text-stone-900 font-medium">Install</span>
          </div>
          <h1 className="text-2xl font-bold text-stone-900 dark:text-stone-100 tracking-tight">Install — รายการ Project</h1>
        </div>
        <button
      ...
      <div className="bg-white dark:bg-stone-800 rounded-2xl border border-stone-200 dark:border-stone-700 shadow-sm p-4 mb-5">
        <p className="text-xs font-semibold text-stone-400 uppercase tracking-wide mb-2.5">ช่วงวันที่ (ค่าเริ่มต้น: วันนี้)</p>
        ...
        <p className="text-xs font-semibold text-stone-400 uppercase tracking-wide mb-2.5">ค้นหาแยกตามฟิลด์</p>
        ...
        <div className="flex items-center gap-2.5 mt-3">
          <button className="px-4 py-2.5 rounded-xl text-sm font-medium text-white bg-stone-900 hover:bg-stone-800 dark:bg-stone-100 dark:text-stone-900 dark:hover:bg-white shadow-sm">ค้นหา</button>
          <button onClick={resetFilters} className="px-4 py-2.5 rounded-xl text-sm font-medium text-stone-500 dark:text-stone-400 border border-stone-200 dark:border-stone-700 hover:bg-stone-50 dark:hover:bg-stone-800">ล้างตัวกรอง</button>
          <button className="px-4 py-2.5 rounded-xl text-sm font-medium text-stone-500 dark:text-stone-400 border border-stone-200 dark:border-stone-700 hover:bg-stone-50 dark:hover:bg-stone-800">Export</button>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-stone-800 rounded-2xl border border-stone-200 dark:border-stone-700 shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-stone-100 dark:divide-stone-700">
            ...
            {rows?.map((r) => (
              <tr key={r.id} onClick={() => navigate(`/project/${r.id}`)} className="cursor-pointer hover:bg-stone-100/50 dark:hover:bg-stone-800/5 transition-colors">
                <td className="px-4 py-3 text-stone-500">{r.created_at ? new Date(r.created_at).toLocaleDateString("th-TH") : "-"}</td>
                <td className="px-4 py-3 font-medium text-stone-900">{r.project_number}</td>
                <td className="px-4 py-3 text-stone-700 dark:text-stone-200">{r.site?.name || "-"}</td>
                <td className="px-4 py-3 text-stone-600 dark:text-stone-300">{r.customer?.display_name || "-"}</td>
                <td className="px-4 py-3 text-stone-500">{r.plan || "-"}</td>
                <td className="px-4 py-3 text-stone-500">{r.salesman?.name || "-"}</td>
