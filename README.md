...
## 46. แก้ไขคลังสินค้าได้ + ล็อกรหัสกระเป๋าเงิน (migration 0024)

### 46.1 คลังสินค้า

หน้าตั้งค่า → คลังสินค้า มีปุ่มดินสอกับถังขยะทุกแถว แก้ได้: ชื่อ · ประเภท ·
**ที่อยู่ · เบอร์ติดต่อ · หมายเหตุ** (คอลัมน์ใหม่) · เปิด/ปิดใช้งาน

**ลบได้เฉพาะคลังที่ไม่เคยมีของและไม่เคยมีการเคลื่อนไหว** — `deleteStockLocation()`
นับ `stock_balances` และ `stock_transactions` ก่อน ถ้าเคยใช้แล้วต้องปิดใช้งานแทน
ไม่งั้นประวัติสต็อกจะชี้ไปที่คลังที่ไม่มีอยู่จริง

### 46.2 ล็อกรหัสกระเป๋าเงิน — บังคับที่ฐานข้อมูล ไม่ใช่ซ่อนปุ่ม

คนที่ถือเงินตั้งรหัสไว้ที่กระเป๋าของตัวเอง ต้องใส่รหัสถึงจะแก้หรือลบได้

**สามชั้นที่ทำให้กันได้จริง:**

1. **รหัสเก็บเป็น hash (bcrypt)** ไม่เคยเก็บตัวเลขจริง — คนที่เปิดฐานข้อมูลดูก็อ่านไม่ออก
2. **ตารางที่เก็บ hash ไม่มี policy ให้อ่าน** ดึงออกมาจากฝั่ง client ไม่ได้เลย
   เข้าถึงได้ทางฟังก์ชัน `security definer` เท่านั้น ซึ่งคืนค่าแค่ true/false
3. **ถอด policy update/delete ของตาราง `wallets` ออก** — แก้ตรงๆ ไม่ได้แล้ว
   ทางเดียวคือผ่าน `update_wallet_secure()` / `delete_wallet_secure()` ที่ตรวจรหัสให้ก่อน

ข้อ 3 สำคัญที่สุด ถ้าไม่ทำ คนที่รู้วิธียิง API ตรงจะข้ามรหัสได้ทันที
(0016 เคยให้ `FOR ALL` ไว้ จึงต้องถอดออกในไฟล์นี้)

**กฎที่ตั้งไว้**

| การทำงาน | ต้องมี |
|---|---|
| แก้กระเป๋า | สิทธิ์บัญชี (Admin ขึ้นไป) + รหัสถูก |
| ลบกระเป๋า | Manager ขึ้นไป + รหัสถูก + **ไม่มีรายการเงินผูกอยู่** |
| เปลี่ยนรหัส | รหัสเดิม หรือเป็น Super Admin |
| ยกเลิกการล็อก | รหัสเดิม หรือเป็น Super Admin |

**Super Admin รีเซ็ตรหัสให้ได้** — กันกรณีคนถือเงินลาออกแล้วไม่มีใครเข้าถึงกระเป๋านั้นได้เลย
การรีเซ็ตถูกบันทึกในประวัติว่าเป็น Super Admin ทำ

**ประวัติการแก้ไข** (`wallet_audit`) เก็บว่าใครแก้อะไรเมื่อไร โดยเฉพาะการแก้ยอดยกมา
บันทึกทั้งค่าเดิมและค่าใหม่ — เงินของบริษัทต้องตอบได้ว่าตัวเลขเปลี่ยนเพราะใคร

กระเป๋าที่ปิดใช้งานยังเห็นในหน้าจัดการ (เพื่อเปิดกลับได้) แต่หายจากตัวเลือกตอนบันทึกรายการ

### 46.3 ผลตรวจ

`t10.ts` ตรวจกฎรหัสและสิทธิ์ 19 เคส ผ่านทั้งหมด รวมถึงเคสที่ต้องกันให้ได้:
Sale ที่รู้รหัสก็ยังแก้ไม่ได้ · Admin ลบกระเป๋าไม่ได้ · ลบกระเป๋าที่มีรายการเงินไม่ได้

## 47. ส่วนลดรายบรรทัด — ต่อชิ้น / ทั้งบรรทัด / เปอร์เซ็นต์ (migration 0025)

### 47.1 ปัญหา: ระบบกับคนกรอกเข้าใจช่องเดียวกันคนละแบบ

ช่อง "ส่วนลด" อยู่ถัดจาก "ราคา/หน่วย" คนจึงกรอกเป็น **ลดต่อชิ้น** เป็นธรรมชาติ
แต่ระบบคิดเป็น **ลดทั้งบรรทัด** ตามที่ FlowAccount ทำ

เมื่อจำนวนเยอะ ผลต่างมหาศาล — ไม่ใช่แค่บรรทัดสรุปที่ผิด **มูลค่ารายบรรทัดก็ผิดด้วย**:

| สินค้า | จำนวน | ราคา | ลด | ระบบคิด | ที่ตั้งใจ (ต่อชิ้น) |
|---|---|---|---|---|---|
| HD-F9-AL-90-P | 1,647 | 990 | 630 | 1,629,900 | **592,920** |
| HD-F9-AL-89-P | 7,137 | 580 | 380 | 4,139,080 | **1,427,400** |
| MN-WP-AL-01-P | 549 | 350 | 200 | 191,950 | **82,350** |

ยอดรวมทั้งใบต่างกันจาก 8.13 ล้าน เหลือ 2.10 ล้าน

### 47.2 ทำไมไม่เปลี่ยนไปเป็นต่อชิ้นทั้งหมด

เพราะ **หน้าพิมพ์คำนวณยอดใหม่จากรายการทุกครั้งที่เปิดเอกสาร** ไม่ได้อ่านยอดที่เก็บไว้
ถ้าเปลี่ยนสูตรทิ้งไปเลย เอกสารที่ออกเลขและส่งลูกค้าไปแล้วจะพิมพ์ออกมาเป็นยอดใหม่
ซึ่งไม่ตรงกับใบที่ลูกค้าถืออยู่ — ผิดกฎหมาย

และการลดทั้งบรรทัดก็ถูกต้องในบางกรณีจริง (ใบ QT ของ FlowAccount ที่ใช้อ้างอิงตั้งแต่ §B3
คือ `2 × 14,900 − 17,800 = 12,000` ซึ่งเป็นลดทั้งบรรทัด)

### 47.3 วิธีแก้: เก็บ "โหมด" กับ "ค่าที่กรอก" แยกจากยอดที่หักจริง
...
- ปุ่มข้างช่องส่วนลดสลับได้ 3 โหมด แสดงป้าย `฿/ชิ้น` · `฿ รวม` · `%`
- **บรรทัดใหม่ตั้งต้นเป็น `฿/ชิ้น`** ตามที่คนกรอกเข้าใจ
- ใต้ช่องแสดง **"หักจริง −x,xxx"** เสมอ ไม่ว่าโหมดไหน — ตัวเลขนี้คือสิ่งที่กันเข้าใจผิดซ้ำ
- แปลงเอกสารต่อกันยกโหมดตามไปด้วย ยอดใบลูกจึงตรงใบต้นทาง

**เอกสารเก่าไม่ขยับแม้แต่บาทเดียว** — migration backfill แถวเดิมเป็นโหมดที่ตรงกับ
พฤติกรรมเดิม (`percent` ถ้าเคยกรอกเป็น % ที่เหลือเป็น `line`) และ `lineDiscount()`
ยังมีทางคำนวณแบบเดิมสำหรับแถวที่ไม่มีโหมด

### 47.4 ผลตรวจ

`t11.ts` ตรวจ 6 หมวดด้วยตัวเลขจากหน้าจอจริง ผ่านทั้งหมด รวมถึงข้อสำคัญ:
**โหมด `line` ให้ผลเท่าของเดิมเป๊ะ** และ **เอกสารเก่าที่ไม่มีโหมดยังได้ยอดเดิม**
(ใบ QT ของ FlowAccount ยังได้ 12,000)

`t_core.ts` ตรวจยอดของเอกสารจริงทั้ง 3 ใบ + ลูกโซ่ QT→BL→INV + หัก ณ ที่จ่ายรายบรรทัด
ผ่านครบ ไม่มี regression

## 48. ราคาสุทธิต่อหน่วยบนเอกสาร + ผลตรวจ logic ใบเสนอราคา

### 48.1 คอลัมน์ "ราคาสุทธิ/หน่วย"

เพิ่มทั้งในหน้าสร้างเอกสารและบนใบที่พิมพ์ คิดจาก `มูลค่าบรรทัด ÷ จำนวน`
จึงถูกต้องทุกโหมดส่วนลด (ต่อชิ้น / ทั้งบรรทัด / %) โดยไม่ต้องแยกสูตร

ผลคือลูกค้ากระทบยอดเองได้บนหน้าเดียว: **จำนวน × ราคาสุทธิ/หน่วย = มูลค่า**
เดิมต้องเอา 490 − 170,190÷549 มาคิดเองถึงจะรู้ว่าราคาจริงคือ 180

### 48.2 บั๊กที่เจอตอนตรวจ: คอลัมน์ส่วนลดว่างตอนดูตัวอย่างก่อนบันทึก

`patchItem()` อัปเดตแค่ `line_total` ไม่ได้อัปเดต `discount_amount`
เอกสารที่บันทึกแล้วไม่มีปัญหาเพราะโหลดค่าจากฐานข้อมูล แต่ **ร่างที่ยังไม่บันทึก
คอลัมน์ "ส่วนลด" บนใบจะว่าง** ทั้งที่ยอดรวมถูก

แก้สองชั้น: หน้าจออัปเดต `discount_amount` ทุกครั้งที่แก้บรรทัด และหน้าพิมพ์
เปลี่ยนไปเรียก `lineDiscount()` ตัวเดียวกับที่ใช้รวมยอด — ตัวเลขบนใบกับยอดสรุป
จึงมาจากแหล่งเดียวกันเสมอ ไม่มีทางเพี้ยนจากกัน

### 48.3 เพิ่มป้ายบอกโหมดราคา

ใต้บรรทัด VAT มีข้อความเล็กๆ **"(ราคาต่อหน่วยรวมภาษีแล้ว)"** หรือ
**"(ราคาต่อหน่วยยังไม่รวมภาษี)"** — กันเถียงกันทีหลังว่าราคาที่เสนอรวม VAT หรือยัง
ซึ่งเป็นข้อพิพาทที่เกิดบ่อยเพราะระบบรองรับทั้งสองโหมด

### 48.4 ผลตรวจ logic ทั้งใบ

`audit_qt.ts` ตรวจว่าตัวเลขทุกตัวบนใบกระทบยอดกันได้ ด้วยเลขจากเอกสารจริง 6 บรรทัด:

- ผลรวมคอลัมน์ "มูลค่า" = จำนวนเงินหลังหักส่วนลด
- รวมเป็นเงิน − ส่วนลด = จำนวนเงินหลังหักส่วนลด
- ฐานภาษี + VAT = จำนวนเงินรวมทั้งสิ้น
- จำนวน × ราคาสุทธิ/หน่วย = มูลค่า ครบทุกบรรทัด
- บรรทัดยกเว้นภาษีแยกออกจากฐานถูกต้อง · แบ่งชำระ % ย่อทุกส่วนพร้อมกัน
- จำนวนเงินเป็นตัวอักษรตรงกับยอดชำระ · ส่วนลดไม่ทำให้ยอดติดลบ

`recon.ts` สุ่มเอกสาร **20,000 ใบ** (จำนวน ราคา และโหมดส่วนลดสุ่มทั้งหมด
ทั้งโหมดราคารวม VAT และไม่รวม) เพื่อหาเคสที่ผลรวมคอลัมน์มูลค่าไม่ตรงกับยอดสรุป
จากการปัดเศษสะสม — **ไม่พบสักเคส**

ไม่พบจุดที่ผิดหลักบัญชีเพิ่มเติม ชุดทดสอบเดิมทั้งหมดยังผ่านครบ

## 49. คอลัมน์ส่วนลดบนใบเสนอราคาแสดงเป็นต่อหน่วย

เดิมคอลัมน์ "ส่วนลด" แสดงยอดรวมของบรรทัด (170,190.00) ซึ่งอยู่ติดกับ "ราคา/หน่วย" (490.00)
ลูกค้าอ่านแล้วบวกลบตามไม่ได้ ต้องเอา `490 − 170,190 ÷ 549` มาคิดเองถึงจะรู้ว่าราคาจริงคือ 180

เปลี่ยนเป็น **ส่วนลด/หน่วย เป็นตัวเลขหลัก** และมี **"รวม x,xxx"** เป็นบรรทัดเล็กใต้ลงไป

```
ราคา/หน่วย   ส่วนลด/หน่วย   ราคาสุทธิ/หน่วย    มูลค่า
   490.00        310.00          180.00      98,820.00
                 รวม 170,190.00
```

ตรวจได้สองทางในบรรทัดเดียว:
`ราคา/หน่วย − ส่วนลด/หน่วย = ราคาสุทธิ/หน่วย` และ `จำนวน × ราคาสุทธิ/หน่วย = มูลค่า`
ส่วนบรรทัด "รวม" ของทุกแถวบวกกันได้เท่ากับ "ส่วนลด" ในสรุปยอดด้านล่างพอดี

ส่วนลด/หน่วยคิดจาก `lineDiscount() ÷ จำนวน` จึงถูกทุกโหมด — โหมดต่อชิ้นได้ค่าที่กรอกกลับมาตรงๆ
โหมดทั้งบรรทัดและ % ได้ค่าเฉลี่ยต่อหน่วยที่กระทบยอดได้เหมือนกัน

ปรับความกว้างคอลัมน์ด้วย: ตัวเลขต่อหน่วยสั้นกว่ายอดรวมมาก จึงคืนพื้นที่ ~14mm
ให้ช่องรายละเอียดสินค้า ชื่อรุ่นที่เคยตัดเป็น 3 บรรทัดจะพอดีขึ้น

`t12.ts` ตรวจครบทุกบรรทัดของใบจริง ทุกโหมดส่วนลด และการกระทบยอดกับสรุป — ผ่านทั้งหมด

## 50. เบอร์ผู้ขายบนเอกสาร · จัดหัวเอกสารใหม่ (migration 0026)

### 50.1 เบอร์โทรของเซลล์

`users.phone` ใหม่ — แก้ได้ที่ **การตั้งค่า → User & Role Management** (ปุ่มดินสอ)
พร้อมแก้ชื่อที่แสดงได้ในที่เดียวกัน

เก็บที่ผู้ใช้ ไม่ใช่ที่เอกสาร โดยตั้งใจ — เอกสารเก็บ snapshot เฉพาะบริษัทและลูกค้า
เพราะสองอย่างนั้นต้องแช่แข็งตามกฎหมาย ส่วนเบอร์เซลล์เปลี่ยนแล้วใบเก่าควรขึ้นเบอร์ปัจจุบัน
ลูกค้าจะได้โทรติด

ผู้ใช้ที่ยังไม่ใส่เบอร์ขึ้นป้ายเตือนในหน้าตั้งค่าว่าเอกสารจะไม่มีเบอร์ให้ลูกค้าโทรกลับ

### 50.2 จัดหัวเอกสารใหม่

- **ตัด "ประเภทงาน" ออกจากเอกสารที่พิมพ์** — เป็น Tag สำหรับจัดกลุ่มและรวมยอดภายใน
  ลูกค้าไม่ต้องเห็น (ยังใช้กรองและรวมยอดในระบบเหมือนเดิมทุกอย่าง)
- **ย้ายผู้ติดต่อ/เบอร์โทรของลูกค้าไปฝั่งซ้าย ใต้บล็อกลูกค้า** — เป็นข้อมูลของฝั่งเดียวกัน
  ฝั่งขวาเหลือเฉพาะข้อมูลของตัวเอกสาร สองคอลัมน์จึงสูงใกล้กัน
- **เพิ่ม "เบอร์โทร" ใต้ "ผู้ขาย"** ฝั่งขวา

### 50.3 ตรวจว่าเอกสารทั้งสายใช้ฟอร์มเดียวกัน

ยืนยันด้วยโครงสร้างโค้ด ไม่ใช่ด้วยการเปิดดูทีละใบ:

- **มีคอมโพเนนต์พิมพ์ตัวเดียวทั้งระบบ** (`DocumentPrintView`) ถูกเรียกจากหน้าเดียว
  (`DocumentEditorPage`) ซึ่งรับทุกประเภทผ่าน route `/accounting/:docType`
- **ชุดข้อมูลที่ส่งไปพิมพ์ (`printable`) สร้างเหมือนกันทุกประเภท** — ตรวจแล้วไม่มีเงื่อนไข
  `docType` อยู่ในนั้นเลย การแก้หัวเอกสารรอบนี้จึงมีผลกับ QT / BL / INV / RC / CN / DN / PO
  พร้อมกันทั้งหมดโดยอัตโนมัติ

ความต่างตามประเภทมีเท่าที่ตั้งใจไว้ และเป็นไปตามธรรมเนียมเอกสารไทย:

| ประเภท | ต่างตรงไหน |
|---|---|
| QT | ยืนราคาถึง · ลายเซ็น "ผู้สั่งซื้อสินค้า / ผู้อนุมัติ" |
| BL · PO | ครบกำหนด · ลายเซ็น "ผู้รับวางบิล / ผู้วางบิล" |
| INV | "ต้นฉบับ (เอกสารออกเป็นชุด)" ตามกฎหมาย · วันที่รับชำระ · บล็อกวิธีชำระเงิน |
| RC | วันที่รับชำระ · บล็อกวิธีชำระเงิน |

ชุดทดสอบยอดเงินทั้ง 4 ชุดยังผ่านครบ (รอบนี้ไม่แตะสูตรคำนวณเลย)

## 51. ล็อกเอกสารให้อยู่ใน A4 เสมอ — ตัดหน้าด้วยการวัดความสูงจริง

### 51.1 ทำไมของเดิมล้นหน้า

ตัวจัดหน้าเดิมนับ **"จำนวนบรรทัด"** ตายตัว (หน้าแรก 11 แถว หน้าถัดไป 20 แถว)
ซึ่งเดาไม่ได้จริง เพราะรายละเอียดสินค้าเป็นข้อความหลายบรรทัดและภาษาไทยตัดคำไม่เท่ากัน
สินค้าอย่าง `Hyundai Switch - 1 gang 2 way` กิน 2–3 บรรทัด ในขณะที่ตัวนับคิดว่า 1 แถว

`.doc-page` ตั้งแค่ `min-height: 297mm` ไม่มีเพดาน เนื้อหาจึงยืดเลยขอบกระดาษ
แล้วเบราว์เซอร์ตัดหน้าเองตรงไหนก็ได้ พอชนกับ `page-break-after: always` เลยได้
หน้าว่างใหญ่ๆ กับลายเซ็นหลุดไปอยู่ลำพัง — ตรงกับที่เห็นในภาพ

### 51.2 วิธีใหม่: วัดก่อน แล้วค่อยจัดหน้า

เรนเดอร์ทุกชิ้นลงชั้นที่ซ่อนไว้ (`position: absolute; left: -9999px` + `no-print`)
กว้างเท่าเนื้อหาจริง 186mm แล้ววัดความสูงจริงจากเบราว์เซอร์ด้วย `getBoundingClientRect()`
ใน `useLayoutEffect` (ทำงานก่อนวาดจอ จึงไม่เห็นการกระพริบ) แล้วค่อยจัดลงหน้า

**เอกสารถูกแตกเป็นชิ้น** แล้วจัดลงหน้าแบบไล่เติมจนเต็ม:
แถวรายการแต่ละแถว · สรุปยอด · หมายเหตุทีละบรรทัด · เงื่อนไขทีละบรรทัด ·
บล็อกวิธีชำระเงิน · ลายเซ็น · ข้อมูลธนาคาร

**ผลที่ได้ตรงกับที่ขอ:**
- **หัวเอกสารซ้ำทุกหน้า** (จำเป็น แต่ละแผ่นต้องอ่านได้ด้วยตัวเอง — FlowAccount ก็ทำแบบนี้)
- **ที่เหลือขึ้นครั้งเดียวทั้งหมด** เงื่อนไขไม่ซ้ำสองหน้า ลายเซ็นไม่ซ้ำสองหน้า
  เพราะแต่ละชิ้นถูกจัดลงหน้าใดหน้าหนึ่งเท่านั้นโดยโครงสร้าง ไม่ใช่ด้วยเงื่อนไข if
- **รายการเยอะ เงื่อนไขไหลไปหน้าถัดไปเอง** และไหลทีละบรรทัด ไม่ตัดกลางประโยค
- **หัวข้อไม่ค้างท้ายหน้า** — "เงื่อนไข" จะไม่อยู่บรรทัดสุดท้ายโดยไม่มีเนื้อหาตาม
  (`keepWithNext` เช็คว่าหัวข้อกับบรรทัดแรกลงหน้าเดียวกันได้ไหมก่อนวาง)
- **ลายเซ็นอยู่หน้าสุดท้ายเสมอ**

### 51.3 ไม่ต้องจำกัดจำนวนตัวอักษร

พี่เสนอมาว่าถ้าต้องกำหนดเพดานความยาวเงื่อนไขให้บอกก่อน — **ไม่ต้องครับ**
การวัดความสูงจริงแก้ที่ต้นเหตุ เงื่อนไขยาวแค่ไหนก็ไหลไปหน้าถัดไปเอง
ไม่มีการตัดข้อความทิ้ง ซึ่งอันตรายกว่ามากสำหรับเอกสารที่มีผลผูกพัน

### 51.4 กันล้นสองชั้น

1. ตัวจัดหน้าเผื่อ **6mm** ไว้กันคลาดจากการวัดเศษ pixel
2. `.doc-page` เปลี่ยนจาก `min-height` เป็น **`height: 297mm` + `overflow: hidden`**
   ล้นไม่ได้เด็ดขาดแม้การวัดผิดพลาด

ชั้นที่สองเป็นเพียงกันชน ไม่ใช่ตัวทำงานหลัก — ถ้าตัวจัดหน้าทำงานถูก ไม่มีอะไรถูกตัด

### 51.5 ผลตรวจ

`paging.ts` จำลองตัวจัดหน้าตัวเดียวกัน ทดสอบ 8 ขนาดเอกสาร ตั้งแต่ 1 รายการ
จนถึง **200 รายการ (13 หน้า)** และเคสเงื่อนไขยาว 120 บรรทัด ตรวจ 4 ข้อทุกเคส:

- ไม่มีหน้าไหนล้น
- ไม่มีบล็อกซ้ำสองหน้า
- ไม่มีบล็อกตกหล่น
- ลายเซ็นอยู่หน้าสุดท้ายเสมอ

ผ่านทั้งหมด 32 การตรวจ ชุดทดสอบยอดเงินเดิมยังผ่านครบ (รอบนี้ไม่แตะสูตรคำนวณ)

## 52. เปลี่ยนแบรนด์เป็น ARCA HAUS + ธีมขาวดำ

### 52.1 ตราประทับบริษัท

วาดเป็น **SVG** (`components/brand/ArcaSeal.tsx`) ไม่ได้ฝังไฟล์รูป เพราะ:

- คมทุกขนาด ตั้งแต่ favicon 32px จนถึงหัวเอกสารที่พิมพ์ 300dpi
- ใช้ `currentColor` จึงกลับสีเองในโหมดมืด ไม่ต้องมีไฟล์สองชุด
- ไม่มีขอบขาวติดมาเวลาวางบนพื้นสี และเล็กกว่ารูปหลายสิบเท่า

ข้อความโค้งตามวงใช้ `textPath` — ส่วนโค้งบนวิ่งซ้าย→ขวาแบบตามเข็ม (sweep 1)
ส่วนโค้งล่างวิ่งซ้าย→ขวาแบบทวนเข็ม (sweep 0) ตัวอักษรจึงตั้งตรงทั้งบนและล่าง

ใช้ที่: sidebar · หน้า login · **หัวเอกสารที่พิมพ์ทุกใบ** · favicon (`public/seal.svg`)

ชื่อระบบเปลี่ยนเป็น **ARCA HAUS — E-Service** ทั้ง title, sidebar, login และ `package.json`

### 52.2 ธีมขาวดำ

**สีที่เหลือในระบบต้องมีหน้าที่เท่านั้น** — ที่เหลือเป็นเทาทั้งหมด
ความสำคัญสื่อด้วยน้ำหนักตัวอักษรและเส้นแบ่ง แทนการใช้สี

| สี | หน้าที่ | จำนวนจุด |
|---|---|---|
| แดง | ผิดพลาด · เงินออก · ลบข้อมูล | 202 |
| เหลือง | รอดำเนินการ · เตือน | 84 |
| เขียว | สำเร็จ · เงินเข้า | 63 |

เปลี่ยนไปแล้ว: **indigo 307 จุด → เทา** และสีตกแต่ง (sky/teal/blue/pink/violet/purple)
อีก 30 จุด → เทา · พื้นแอปจาก `slate-50` (อมฟ้า) เป็นขาวสนิท · โหมดมืดจาก `slate-950`
เป็นดำเป็นกลาง `#0a0a0a` · ป้ายประเภทงานเป็นชิปเส้นขอบเรียบเหมือนกันหมด

**ปุ่มหลักกลับด้านในโหมดมืด** — `bg-slate-900` บนพื้นดำจะมองไม่เห็น
จึงเติม `dark:bg-slate-100 dark:text-slate-900` ให้ทั้ง 46 จุด

### 52.3 เอกสารที่พิมพ์ยังเป็นสีเหมือนเดิม

ธีมขาวดำใช้กับ **หน้าจอระบบเท่านั้น** เอกสารที่พิมพ์คงของเดิมไว้ทั้งหมด —
สีประจำประเภท (QT ส้ม · BL ม่วง · INV ฟ้า) และสามเหลี่ยมมุมกระดาษ 26mm

เหตุผล: กระดาษที่ส่งลูกค้าคนละบริบทกับหน้าจอที่ทีมใช้ทำงาน ในกองเอกสารบนโต๊ะ
สีมุมกระดาษช่วยหยิบใบที่ต้องการได้เร็วกว่าอ่านหัวเรื่อง

**สิ่งเดียวที่เพิ่มบนเอกสารคือตราประทับบริษัท** วางไว้หน้าชื่อบริษัทมุมบนซ้าย

### 52.4 บั๊กที่ตัวตรวจจับได้

หลังแก้สียกชุด ไล่ตรวจไฟล์ `.jsx`/`.tsx` ทั้งโปรเจกต์ว่าคอมโพเนนต์ที่ใช้ถูก import ครบ
เจอ **`LoginPage.jsx` เรียก `ArcaSeal` แต่ import ไม่ติด** — เป็นบั๊กชนิดเดียวกับ §42
ที่ทำให้หน้าจอขาว และ `npm run build` จับไม่ได้เพราะไฟล์ `.jsx` ไม่ได้อยู่ใน typecheck

ชุดทดสอบยอดเงินและการตัดหน้าทั้ง 5 ชุดยังผ่านครบ (รอบนี้ไม่แตะตรรกะเลย)

## 53. ปรับเอกสารตาม feedback จากทีม

แก้ตามที่ทีมมาร์กมาบนใบ QT202608002 ทั้ง 6 จุด (ที่อยู่ไม่ต้องแก้ในโค้ด — แก้ที่หน้าตั้งค่าได้เอง)

### 53.1 หัวเอกสาร

| เดิม | ใหม่ |
|---|---|
| ซ้ายใต้ชื่อไทย: `QUOTATION` | **ชื่อบริษัทภาษาอังกฤษ** `AR-CA HAUS CO., LTD.` |
| ขวา: `ใบเสนอราคา` อย่างเดียว | `ใบเสนอราคา` + **`(QUOTATION)`** ใต้ลงไป |

ชื่อภาษาอังกฤษของเอกสารควรอยู่คู่ชื่อไทยของเอกสาร ไม่ใช่ไปอยู่ใต้ชื่อบริษัท
ตำแหน่งเดิมทำให้อ่านผิดว่าบริษัทชื่อ "QUOTATION"

ต้องเพิ่ม `name_en` ลงใน snapshot ของบริษัท (`PartySnapshot.name_en`)
เอกสารเก่าที่ออกไปแล้วจะยังไม่มีชื่ออังกฤษ เพราะ snapshot ถูกแช่แข็งไว้ตอนออกเลข —
ใบใหม่มีครบ

### 53.2 รวมบรรทัดที่แยกโดยไม่จำเป็น

- **ผู้ขาย + เบอร์โทร** เหลือบรรทัดเดียว `ผู้ขาย  Pear   099-9999999`
- **ผู้ติดต่อ + เบอร์โทร** เหลือบรรทัดเดียว `ผู้ติดต่อ คุณฝน …  โทร. -`

ประหยัดพื้นที่หัวเอกสารไป 2 บรรทัด และอ่านเป็นชุดเดียวกันตามที่มันเป็นจริง

### 53.3 ตัดตัวเลขที่ซ้ำซ้อน

- **ตัดบรรทัด "รวม x,xxx" ใต้ส่วนลด/หน่วย** — ยอดรวมส่วนลดมีอยู่ในสรุปยอดอยู่แล้ว
  และคอลัมน์ "มูลค่า" ก็บอกผลลัพธ์แล้ว ตัวเลขซ้อนใต้ช่องทำให้ตารางอ่านยากขึ้นเปล่าๆ
- **ตัดคำกำกับ "(ราคาต่อหน่วยยังไม่รวมภาษี)"** ใต้บรรทัด VAT — สรุปยอดแยก
  "มูลค่าที่คำนวณภาษี" กับ "ภาษีมูลค่าเพิ่ม" ให้เห็นชัดอยู่แล้ว ไม่ต้องอธิบายซ้ำ

### 53.4 ข้อมูลธนาคารย้ายมาอยู่ข้างสรุปยอด

เดิมอยู่ท้ายสุดของเอกสาร ตอนนี้อยู่ **ฝั่งซ้ายระดับเดียวกับสรุปยอด** ซึ่งเป็นช่องว่าง
ที่เดิมปล่อยทิ้ง — ลูกค้าเห็นเลขบัญชีกับยอดที่ต้องโอนในสายตาเดียว

โครงสร้างเปลี่ยนจาก block แยก (`kind: 'bank'`) มาอยู่ในบล็อกสรุปยอด
ตัวจัดหน้าจึงวัดความสูงรวมกันทีเดียว ไม่มีทางหลุดไปคนละหน้ากับยอดเงิน

ชุดทดสอบทั้ง 5 ชุดยังผ่านครบ (ยอดเงินและการตัดหน้าไม่ถูกแตะ)

## 54. ใบเสนอราคา — layout ใหม่ตามแบบที่ทีมกำหนด

### 54.1 โลโก้เปลี่ยนเป็น wordmark

`components/brand/ArcaWordmark.tsx` — คำว่า ARCA วาดเป็น **SVG path ไม่ใช่ `<text>`**
เพราะถ้าใช้ text แล้วเครื่องที่เปิดไม่มีฟอนต์เดียวกัน รูปตัวอักษรจะเพี้ยน
โลโก้บริษัทบนเอกสารที่ส่งลูกค้าเพี้ยนไม่ได้

ตราประทับวงกลม (`ArcaSeal`) ยังใช้ในระบบ (sidebar · login · favicon) แต่ไม่ใช้บนเอกสารแล้ว

### 54.2 หัวเอกสาร

```
[ARCA]  บริษัท อาร์-คา เฮาส์ จำกัด            ใบเสนอราคา
        AR-CA HAUS CO., LTD.                    ต้นฉบับ
        27/207 ถนนหนองระแหง …                  หน้าที่ 1/3
        เลขประจำตัวผู้เสียภาษี …   E-mail …

ลูกค้า                                  เลขที่      QT202609001
บริษัท ฟอร์เรสทีเรีย จำกัด (สำนักงานใหญ่)   วันที่       01/09/2026
เลขประจำตัวผู้เสียภาษี …                  ยืนราคาถึง   31/12/2026
ผู้ติดต่อ -    โทร. -                     ผู้ขาย      บอส  0863901916

ชื่องาน    Project Forest Cafe - ระบบ Hotel Lock & Access Control
```

- ตัด block "ผู้ออกเอกสาร" ที่มีหัวข้อกำกับออก — ข้อมูลบริษัทอยู่ในแถวโลโก้เลย
- **ชื่องานย้ายมาเป็นแถวเต็มความกว้าง** ชื่อโครงการมักยาว เดิมบีบอยู่ในคอลัมน์ขวา 72mm
- เพิ่ม E-mail บริษัท (ดึงจาก `companies.email` ที่มีอยู่แล้ว)
- `หน้าที่ x/y` ขึ้นเสมอ ไม่ใช่เฉพาะตอนหลายหน้า

### 54.3 ตารางเหลือ 7 คอลัมน์

`ลำดับ · รายละเอียด · จำนวน · หน่วย · ราคา/หน่วย · ส่วนลด · มูลค่า`

ตัดออก 2 คอลัมน์: **ภาษี** (สรุปยอดบอกอยู่แล้ว และทั้งใบใช้อัตราเดียวกันเกือบตลอด)
และ **ราคาสุทธิ/หน่วย**

ช่องส่วนลดยังเป็น**ต่อหน่วย** เพื่อให้บวกลบในบรรทัดเดียวได้:
`(ราคา/หน่วย − ส่วนลด) × จำนวน = มูลค่า`

เส้นคาดหัวตารางหนา 1.5px สีประจำเอกสาร หัวคอลัมน์เป็นสีเดียวกัน · **ไม่มีเส้นคั่นรายแถว**
ตามแบบ

### 54.4 ท้ายเอกสาร

- **"กรุณาชำระ"** ซ้ายล่าง ระดับเดียวกับสรุปยอด — ชื่อบริษัท + ธนาคาร + เลขบัญชี
  (เดิมเป็นการ์ดกรอบสี่เหลี่ยม เปลี่ยนเป็นข้อความเรียบตามแบบ)
- **จำนวนเงินเป็นตัวอักษรชิดขวาใต้ยอดชำระ** เดิมอยู่ชิดซ้ายแยกบรรทัด
- หมายเหตุ/เงื่อนไขมีเส้นคั่นเหนือหัวข้อแรก
- ลายเซ็นห่างขึ้นและกว้างขึ้นตามแบบ

### 54.5 การตัดหน้ายังทำงานเหมือนเดิม

รายการเกินหน้าไหลไปหน้าถัดไปเอง หัวเอกสารซ้ำทุกหน้า ที่เหลือขึ้นครั้งเดียว
ลายเซ็นอยู่หน้าสุดท้าย — ระยะขอบกระดาษเพิ่มจาก 12mm เป็น 14mm ตามแบบ
ตัวจัดหน้าคิดพื้นที่ใหม่ให้อัตโนมัติเพราะวัดความสูงจริง

ชุดทดสอบทั้ง 5 ชุดยังผ่านครบ

## 55. โลโก้จริง · ที่อยู่ลูกค้า · เนื้อหาถูกตัดขอบกระดาษ

### 55.1 โลโก้ไม่ตรง — เลิกวาดเอง ใช้ไฟล์จริง

รอบก่อนวาด ARCA เป็น SVG path เอง ผลคือตัว R กับ C เพี้ยน อ่านเป็น "ARCΛ"
**เปลี่ยนมาใช้ไฟล์โลโก้จริงของบริษัท** — โลโก้บนเอกสารที่ส่งลูกค้าเพี้ยนไม่ได้

`public/logo-arca.png` — แปลงจาก LOGO2.jpg เป็น PNG พื้นโปร่งใส 1200px กว้าง
พิมพ์ที่ 34mm ได้ราว 900dpi คมเกินพอสำหรับงานพิมพ์ และไฟล์แค่ 14KB

> ตอบคำถาม "ต้องแนบรูปไหม" — **ใช่ครับ** โลโก้ที่มีเส้นโค้งเฉพาะตัวแบบนี้
> วาดใหม่ให้ตรงต้นฉบับ 100% ไม่ได้ ต้องใช้ไฟล์จริง

### 55.2 ที่อยู่ลูกค้าไม่ขึ้น

ฟอร์มลูกค้าใน Contact มีช่องที่อยู่ **สองช่อง**: "ที่อยู่วางบิล" (`billing_address`)
และ "ที่ตั้งสำนักงาน" (`office_address`) แต่เอกสารอ่านแค่ `billing_address → address`
**ไม่เคยมองที่ `office_address` เลย**

ทีมกรอกแต่ "ที่ตั้งสำนักงาน" ที่อยู่จึงหายไปทั้งใบ

แก้เป็นลำดับสำรอง `billing_address → office_address → address` —
ที่อยู่วางบิลมาก่อนตามที่เอกสารภาษีต้องการ แต่ถ้าไม่ได้กรอกก็ยังมีที่อยู่ขึ้น
ซึ่งจำเป็น เพราะใบกำกับภาษีที่ไม่มีที่อยู่ผู้ซื้อใช้ไม่ได้ตามกฎหมาย

### 55.3 เนื้อหาถูกตัดขอบกระดาษ — margin ไม่ถูกนับตอนวัด

บรรทัด "วันที่" ใต้ลายเซ็นหายไปในหน้า 2 เพราะ `overflow: hidden` ตัดทิ้ง

**ต้นเหตุ:** `getBoundingClientRect()` ของ div ที่ครอบ **ไม่รวม margin ที่ collapse ออกไป**
บล็อกลายเซ็นมี `mt-12` (48px) จึงถูกวัดว่าสูงราว 55px ทั้งที่จริงกินพื้นที่ ~110px
ตัวจัดหน้าเลยคิดว่ายังเหลือที่ แล้วยัดลงหน้าเดิม

**แก้สามชั้น:**

1. **เลิกใช้ margin แนวตั้งที่รากของทุกบล็อกที่ถูกวัด** เปลี่ยนเป็น padding
   ซึ่งนับรวมใน `getBoundingClientRect()` เสมอ
2. **วัดใหม่เมื่อฟอนต์และรูปพร้อม** — ฟอนต์ไทยและไฟล์โลโก้โหลดเสร็จหลังการวาดครั้งแรก
   ถ้าวัดด้วยฟอนต์สำรองความสูงจะเพี้ยนทั้งใบ (`document.fonts.ready` + `onLoad` ของรูป)
3. **เพิ่มพื้นที่เผื่อจาก 6mm เป็น 10mm**

ผลคือเนื้อหาที่ไม่พอหน้า **ย้ายไปหน้าถัดไป** แทนที่จะถูกตัดหาย ซึ่งเป็นสิ่งที่ต้องเป็น —
เอกสารที่ส่งลูกค้าห้ามมีอะไรหายเงียบๆ

ชุดทดสอบทั้ง 5 ชุดยังผ่านครบ

## 56. จัดกลุ่มรายการในเอกสารขาย (Group สินค้า Type A/B/C)

### 56.1 ปัญหาเดิม

งานตกแต่ง/เฟอร์นิเจอร์ที่แบ่งเสนอราคาเป็นชุด (เช่น Type A / Type B / Type C)
ต้องยัดรายการย่อยทั้งหมด (ชุดครัว, โซฟา, โต๊ะกินข้าว, ...) เป็นข้อความในช่อง
รายละเอียดของบรรทัดเดียว แล้วตั้งราคาเป็นก้อนรวมทั้ง Type — ตั้งราคาแยกแต่ละ
ชิ้นไม่ได้เลย

### 56.2 ทางแก้ — ตารางกลุ่มใหม่ที่ "ครอบ" รายการเดิม ไม่แตะสูตรคำนวณ

เพิ่มตาราง `ar_document_item_groups` (0027) + คอลัมน์ `group_id` บน
`ar_document_items` เป็นแค่ชั้นจัดกลุ่ม/แสดงผล — `computeTotals()` ใน
`accounting-lib/calc.ts` **ไม่ถูกแก้แม้แต่บรรทัดเดียว** ยังรวม subtotal /
vat_base / vat_amount / grand_total จากทุก item ทั้งเอกสารเหมือนเดิมทุก
ประการ ไม่สนใจว่าใครอยู่กลุ่มไหน

ยอดรวมย่อยต่อกลุ่มที่เห็นบนเอกสารคือ `SUM(line_total)` กรองด้วย `group_id`
คำนวณตอนแสดงผลเท่านั้น (ฟังก์ชันใหม่ `buildItemLayout()` ใน `calc.ts` ใช้ร่วม
กันทั้งหน้าแก้ไขและหน้าพิมพ์ เพื่อให้เลขลำดับ/ยอดรวมย่อยตรงกันเป๊ะทั้งสองที่)

**Optional ต่อเอกสาร** — ใบที่ไม่เคยกด "เพิ่ม Group" เลย (`group_id` ทุกแถว
เป็น null) แสดงผลเหมือนก่อนมี migration นี้ทุกประการ ทั้งหน้าจอและหน้าพิมพ์
เอกสารเก่าทั้งหมดไม่ถูกกระทบ

### 56.3 กติกาที่บังคับไว้ในโค้ด

- **ห้ามมีกลุ่มว่าง** — ลบ item สุดท้ายของกลุ่มใดกลุ่มหนึ่ง กลุ่มนั้นถูกลบไป
  ด้วยอัตโนมัติ (ทั้งตอนกดในหน้าจอ และเช็คซ้ำที่ `saveArDocument` กันพลาด
  จากดาต้าที่ไม่ครบ)
- **"ลบกลุ่ม" ไม่ลบข้อมูลสินค้า** — แค่ย้ายรายการทั้งหมดออกมานอกกลุ่ม
  (`group_id` เป็น null) ต้องการลบรายการจริงให้กดลบทีละบรรทัดแยกต่างหาก
- **item ในกลุ่มเดียวกันต้องเรียงติดกันในอาเรย์เสมอ** — ปุ่ม "เพิ่มสินค้าใน
  [ชื่อกลุ่ม]" แทรกบรรทัดใหม่ต่อจากรายการสุดท้ายของกลุ่มนั้นเสมอ ไม่ปล่อยให้
  หลุดตำแหน่ง (ยังไม่มี drag reorder ในรอบนี้ — ไม่จำเป็นเพราะ add/remove
  คงลำดับให้เองอยู่แล้ว)
- **เลขลำดับ (1, 2, 3...) เริ่มใหม่ทุกกลุ่ม** รายการนอกกลุ่มนับต่อกันแบบเดิม
  (เท่ากับตำแหน่งในอาเรย์ — ไม่กระทบ `line_no` จริงที่ใช้คำนวณ/บันทึก)

### 56.4 แปลงเอกสาร QT → BL → INV พากลุ่มไปด้วย

`group_id` เป็น primary key ต่อแถวในตาราง ไม่ใช่ตัวระบุกลุ่มข้ามเอกสาร —
ตอนแปลงเอกสาร (`convertArDocument`) และตอนดึงข้อมูลจากใบต้นทางด้วยตัวเอง
(ปุ่มอ้างอิงใบต้นทางในหน้าแก้ไข, ฟังก์ชัน `applySource`) ต้องออก id ใหม่ให้
ทุกกลุ่มก่อน แล้วชี้ item ที่เคยอยู่กลุ่มเดิมไปกลุ่มใหม่ตาม ไม่งั้นโครงสร้าง
กลุ่มหายหรือไปชนกับกลุ่มของเอกสารอื่นที่บังเอิญมี id เดียวกัน

### 56.5 หน้าพิมพ์ — เพิ่ม 2 บล็อกใหม่เข้าไปในตัวจัดหน้าเดิม

ตัวจัดหน้า (วัดความสูงจริงจากเบราว์เซอร์ก่อนแบ่งหน้า จาก §54–55) รู้จัก
บล็อกใหม่สองแบบคือ `group-header` (แถวหัวกลุ่ม) กับ `group-subtotal` (แถว
สรุปยอดย่อยท้ายกลุ่ม) นับเป็น "แถวในตาราง" เหมือน `row`/`empty` ทุกประการ
(ต้องมี thead ซ้ำถ้าขึ้นหน้าใหม่) กลุ่มที่ยาวเกินหน้าเดียวจะไหลข้ามหน้าไป
เหมือนรายการยาวทั่วไป (แถวหัวกลุ่มกับสรุปยอดอาจไปคนละหน้ากันได้ในเคสสุดโต่ง
— ยอมรับเหมือนที่ข้อความหมายเหตุ/เงื่อนไขยาวๆ ก็ไหลข้ามหน้าอยู่แล้ว)

### 56.6 ไม่ครอบคลุมฝั่งซื้อ (PO/PI/PV/IM) ในรอบนี้

ยังไม่มีความต้องการจัดกลุ่มฝั่งซื้อ — ทำเฉพาะ `ar_document_items` (QT/BL/
INV/RC/CN/DN) เพิ่มให้ฝั่งซื้อภายหลังได้โดยทำตามแบบเดียวกันทุกขั้นตอน

ชุดทดสอบทั้ง 5 ชุดยังผ่านครบ

## 57. ทำสำเนาใบเสนอราคา

### 57.1 ที่มา

งานหลายงานเสนอราคาคล้ายของเดิม ต่างกันแค่ลูกค้าหรือจำนวนไม่กี่บรรทัด
แต่เดิมต้องคีย์ใหม่ทั้งใบทุกครั้ง รวมถึงหมายเหตุ เงื่อนไข และรายการสินค้าทั้งหมด

### 57.2 ใช้งาน — กดได้ 2 ที่

- **หน้ารายการใบเสนอราคา** ปุ่มไอคอนสำเนาท้ายแถว — เร็วที่สุด ไม่ต้องเปิดใบเดิม
- **ในหน้าเอกสาร** ปุ่ม "ทำสำเนา" บนแถบเครื่องมือ

ทั้งสองทางพาไปที่ร่างใบใหม่ทันทีหลังสร้างเสร็จ

### 57.3 ลอกอะไรไปบ้าง

ลอก **ทุกอย่างที่เป็นเนื้อหาของใบ**: ลูกค้า ชื่องาน ผู้ติดต่อ ผู้ขาย ประเภทงาน
เลขที่ PO รายการสินค้าทุกบรรทัดพร้อมส่วนลดและโหมดส่วนลด กลุ่มสินค้า (Type A/B/C
จาก §56) ส่วนลดพิเศษท้ายบิล รูปแบบราคา/VAT หัก ณ ที่จ่ายรายบรรทัด หมายเหตุ
และเงื่อนไข — ยอดรวมของสำเนาจึงเท่าใบเดิมทุกบาท

**ไม่ลอก** สิ่งที่เป็นของเฉพาะใบเดิม: เลขที่เอกสาร (ออกเลขใหม่) · สถานะ
(กลับเป็นร่างเสมอ) · ผู้อนุมัติและวันที่อนุมัติ · ข้อมูลการยกเลิก · ยอดที่ชำระ
มาแล้ว · ไฟล์ PDF ที่เคยพิมพ์

**วันที่เอกสาร** = วันที่ทำสำเนา · **วันยืนราคา** = ของเดิมถ้ายังไม่หมดอายุ
ถ้าหมดแล้วเลื่อนเป็นสิ้นปีปัจจุบัน (ค่าตั้งต้นเดียวกับใบใหม่) — ลอกวันที่หมดอายุ
ไปตรงๆ จะได้ใบที่ใช้ไม่ได้ตั้งแต่วันแรก

### 57.4 จุดที่พลาดแล้วพังเงียบ — `source_document_id` ต้องเป็น null

สำเนาคือ **เอกสารตั้งต้นใบใหม่ ไม่ใช่เอกสารลูกของใบเดิม** ถ้าลอก
`source_document_id` ไปด้วย ระบบจะนับยอดของสำเนาไปกินโควตา "ยอดคงเหลือ" ของ
ใบต้นทาง (กฎห้ามวางบิลเกินจาก §6) แล้วใบแจ้งหนี้ที่ต้องออกจริงจะออกไม่ได้
โดยไม่มีอะไรฟ้องว่าเพราะอะไร — `contract_total` ก็ไม่ลอกด้วยเหตุผลเดียวกัน

กลุ่มสินค้าต้องออก `group_id` ใหม่ทุกกลุ่มแล้วชี้ item ตามไป เหมือนตอนแปลง
เอกสาร (§56.4) ไม่งั้นกลุ่มของสำเนาจะไปผูกกับกลุ่มของใบเดิม

### 57.5 เปิดเฉพาะใบเสนอราคา — จงใจ

ใบแจ้งหนี้และใบกำกับภาษีทำสำเนาไม่ได้ และบล็อกไว้ที่ชั้น API ไม่ใช่แค่ซ่อนปุ่ม
เพราะสำเนาใบกำกับภาษีคือใบกำกับภาษีใบที่สองของการขายครั้งเดียวกันแต่คนละเลขที่
ซึ่งผิดทั้งทางบัญชีและทางกฎหมาย และเอกสารกลุ่มนั้นต้องมีใบต้นทางเสมอเพื่อคุมยอด
ไม่ให้วางบิลเกิน — ถ้าต้องการใบใหม่ให้ใช้ปุ่มแปลงเอกสารจากใบต้นทางตามเดิม

### 57.6 ใบที่ยังแก้ไขได้จะถูกบันทึกก่อนทำสำเนา

สำเนาอ่านข้อมูลจากฐานข้อมูล ไม่ได้อ่านจากหน้าจอ ถ้าไม่บันทึกก่อน สิ่งที่เพิ่ง
พิมพ์ค้างไว้จะไม่ติดไปกับสำเนา — ปุ่มในหน้าเอกสารจึงสั่งบันทึกร่างให้ก่อนเสมอ
เมื่อใบนั้นยังไม่ถูกล็อก (ใบที่ออกเลขแล้วแก้ไม่ได้อยู่แล้ว จึงลอกได้ตรงๆ)

ทำสำเนาได้ทุกสถานะ รวมถึงใบที่อนุมัติไปแล้วหรือยกเลิกไปแล้ว ซึ่งเป็นเหตุผลหลัก
ที่ต้องมีปุ่มนี้

### 57.7 ไม่ต้องรัน migration

ฟีเจอร์นี้เป็นโค้ดล้วน ไม่มีตารางหรือคอลัมน์ใหม่ — ฐานข้อมูลยังอยู่ที่ไฟล์ 0027

ชุดทดสอบทั้ง 5 ชุดยังผ่านครบ และเพิ่มชุดตรวจสำเนา: ยอดรวมทุกช่อง
(subtotal / ส่วนลด / ส่วนลดพิเศษ / VAT / หัก ณ ที่จ่าย / ยอดชำระ) ของสำเนา
ต้องเท่าใบต้นฉบับทุกบาท และโครงสร้างกลุ่มพร้อมเลขลำดับในกลุ่มต้องตรงกันเป๊ะ

## 58. แก้ใบเสนอราคา PDF ขาดตอน · เรียงลำดับกลุ่มได้

### 58.1 อาการ

ใบที่ควรจบใน 2 หน้ากลายเป็น 3 หน้า หน้าแรกใช้พื้นที่ไปแค่ครึ่งเดียวแล้วตัดหน้า
รายการของกลุ่มเดียวกันกระจายข้ามหน้าโดยหน้ากลาง **ไม่มีชื่อกลุ่มกำกับเลย**
และหน้าสุดท้ายคอลัมน์รายละเอียดแคบจนข้อความบรรทัดเดียวถูกตัดเป็น 7 บรรทัด

### 58.2 ต้นเหตุเดียวกันทั้งสามอาการ — ตารางคิดความกว้างคอลัมน์คนละแบบกัน

ตารางรายการใช้ CSS แบบ auto ซึ่งคิดความกว้างคอลัมน์จาก **เนื้อหาที่อยู่ในตารางนั้น**
แต่ระบบมีตารางสองชุด: ชั้นที่ใช้วัดความสูงมี **ทุกแถวของเอกสาร** ส่วนตารางของแต่ละหน้า
มีแค่ **แถวของหน้านั้น** ความกว้างคอลัมน์จึงไม่เท่ากัน → ความสูงที่วัดได้ไม่ใช่ความสูง
ที่พิมพ์จริง → ตัวจัดหน้าคำนวณผิดทั้งใบ

ตัวเร่งคือแถวยอดรวมย่อยของกลุ่ม (§56) ที่ใส่ `whitespace-nowrap` ไว้ในคอลัมน์ "ส่วนลด"
ซึ่งกว้างแค่ 22mm ชื่อกลุ่มยาวอย่าง "รวมEnergy Saving ( ไม่รวมค่าติดตั้งและเดินสาย 4 Core )"
จึงดันคอลัมน์นั้นให้กว้างขึ้นมาก ไปเบียดคอลัมน์รายละเอียดให้แคบลง:
- ในชั้นที่ใช้วัด ทุกแถวเลยถูกวัดว่า**สูงเกินจริง** → ยัดได้น้อยแถวต่อหน้า → หน้าว่างครึ่งหน้า
- ในหน้าที่มีแถวน้อย คอลัมน์รายละเอียด**แคบจนผิดรูป** → ข้อความตัดเป็น 7 บรรทัด

**แก้ที่ราก:** ตรึงความกว้างคอลัมน์ด้วย `<colgroup>` + `table-layout: fixed` ทั้งสองชั้น
(ค่าเดิม 12/auto/16/14/24/22/26mm คงไว้ หน้าตาเอกสารจึงไม่เปลี่ยน) และย้ายป้ายยอดรวมย่อย
ไปกินตั้งแต่คอลัมน์แรกถึงคอลัมน์ส่วนลด ชื่อกลุ่มยาวแค่ไหนก็ไม่ดันตารางอีก

เมื่อค่าที่วัดตรงกับของจริงแล้ว ลดพื้นที่เผื่อจาก 10mm เหลือ 6mm ได้อีก

### 58.3 ตัวจัดหน้ารู้จัก "กลุ่ม" เป็นก้อนเดียว

เดิมตัวจัดหน้าเห็นทุกอย่างเป็นแถวเรียงกัน จะตัดตรงไหนก็ได้ ตอนนี้กลุ่มถูกมัดเป็นหน่วยเดียว
(หัวกลุ่ม + รายการ + ยอดรวมย่อย) แล้วตัดสินใจตามลำดับ:

1. ทั้งกลุ่มลงที่เหลือของหน้านี้ได้ → วางต่อเลย
2. ไม่ลง แต่ลงได้ถ้าขึ้นหน้าใหม่ → **ขึ้นหน้าใหม่แล้ววางทั้งก้อน** ยอมปล่อยท้ายหน้าว่าง
   ดีกว่าแตกกลุ่มเพื่ออุดที่ว่างแล้วคนอ่านต้องพลิกหน้าไปมา
3. กลุ่มใหญ่กว่าหนึ่งหน้าจริงๆ → ยอมแตก แต่มีกติกา:
   - หัวกลุ่มต้องมีรายการตามอย่างน้อย 2 บรรทัดบนหน้าเดียวกัน ห้ามโดดท้ายหน้า
   - ยอดรวมย่อยห้ามอยู่หน้าถัดไปลำพัง ถ้าที่ไม่พอจะ**ดึงรายการสุดท้ายไปด้วย**
   - หน้าถัดไปพิมพ์หัวกลุ่มซ้ำพร้อม **"(ต่อ)"** เพื่อให้รู้ว่ารายการที่เห็นเป็นของกลุ่มไหน
   - เลขลำดับในกลุ่มเดินต่อเนื่องข้ามหน้า ไม่รีเซ็ต

ทั้งหมดคำนวณอัตโนมัติตอน export ไม่มีอะไรต้องตั้งค่า

### 58.4 เรียงลำดับกลุ่มได้

ปุ่มลูกศรขึ้น/ลงบนหัวกลุ่มในหน้าแก้ไข สลับกลุ่มกับบล็อกที่อยู่ติดกันบนจอ (จะเป็นกลุ่มอื่น
หรือรายการนอกกลุ่มก็ได้) ลำดับที่เห็นคือลำดับที่พิมพ์ออกใบเสนอราคา

ต้องสลับที่ **อาเรย์รายการ** ไม่ใช่แค่ `sort_order` เพราะทั้งหน้าจอและหน้าพิมพ์เรียงทุกอย่าง
จากลำดับรายการ ส่วน `sort_order` เป็นเพียงลำดับที่เก็บลงฐานข้อมูล — ขยับแค่อย่างใดอย่างหนึ่ง
แล้วสองฝั่งจะไม่ตรงกัน ตอนบันทึก `sort_order` จึงถูกคำนวณใหม่จากลำดับที่รายการปรากฏจริง
เสมอ ไม่เชื่อลำดับในอาเรย์ `groups` ที่ส่งมา

### 58.5 ที่ตรวจเจอเพิ่มตอนรีเช็คทั้งโมดูล

- **ปุ่ม "เพิ่ม Group" โผล่บนใบสั่งซื้อด้วย** ทั้งที่ฝั่งซื้อไม่มีตารางกลุ่มในฐานข้อมูล
  จัดกลุ่มไปแล้วกลุ่มจะหายเงียบตอนบันทึก — ปิดปุ่มให้เหลือเฉพาะเอกสารขาย
- **เลขลำดับกระโดดเมื่อผสมรายการในกลุ่มกับนอกกลุ่ม** เดิมรายการนอกกลุ่มใช้ตำแหน่งใน
  อาเรย์ทั้งใบ จึงเห็นเป็น 1, 2, [กลุ่ม 1, 2], 5 — แก้ให้รายการนอกกลุ่มเดินเลขของตัวเอง
  ต่อเนื่อง (1, 2, [กลุ่ม 1, 2], 3) เอกสารที่ไม่มีกลุ่มเลยยังได้ 1..n เท่าเดิมทุกประการ
- **กลุ่มที่ยังไม่ได้ใส่สินค้าหายเงียบตอนบันทึก** เพราะบรรทัดที่ไม่มีรายละเอียดถูกกรองทิ้ง
  ตอนนี้บล็อกการบันทึกพร้อมบอกชื่อกลุ่มที่ยังว่าง
- **ชื่อกลุ่มว่าง** พิมพ์ออกมาเป็นหัวกลุ่มเปล่ากับบรรทัด "รวม" ที่ไม่บอกว่ารวมอะไร —
  ใส่ชื่อสำรอง "กลุ่มสินค้า" ให้ตอนบันทึก
- **ยอดรวมย่อยของกลุ่มกับการแบ่งชำระ %**: ยอดรวมย่อยแสดงเต็มจำนวนเสมอ ซึ่งตรงกับ
  บรรทัด "รวมเป็นเงิน" ส่วนการแบ่งชำระมีบรรทัด "แบ่งชำระ X%" แยกให้เห็นอยู่แล้ว
  จึงไม่ขัดกัน — ตั้งใจคงไว้แบบนี้

### 58.6 ไม่ต้องรัน migration

โค้ดล้วน ฐานข้อมูลยังอยู่ที่ไฟล์ 0027

ตรวจด้วยการเรนเดอร์เอกสารจริง (QT202609008) ในเบราว์เซอร์จริง: เหลือ 2 หน้าตามที่ควรเป็น
หน้าแรกใช้พื้นที่ 95% (เดิม ~55%) ทั้งสองกลุ่มอยู่ครบในหน้าเดียวไม่ขาดตอน และไม่มีหน้าไหน
เนื้อหาล้นกรอบ · เคสกลุ่มยาว 34 รายการที่ต้องแตกข้ามหน้า: ทุกหน้ามีหัวกลุ่มกำกับ หน้าต่อมี
"(ต่อ)" เลขลำดับ 1–34 เรียงต่อเนื่อง ยอดรวมย่อยมีครั้งเดียวและไม่อยู่ลำพัง

## 59. เลือกได้ว่าจะพิมพ์ต้นฉบับ สำเนา หรือทั้งชุด

เดิมสั่งพิมพ์ครั้งเดียวได้ทั้ง "ต้นฉบับ" และ "สำเนา" ต่อกันเสมอโดยเลือกไม่ได้
ใบเสนอราคา 2 หน้าจึงกลายเป็น PDF 4 หน้าที่หน้า 3-4 เป็นหน้า 1-2 ซ้ำอีกรอบ
ต่างกันแค่ป้ายมุมขวาบน ทั้งที่งานส่วนใหญ่ส่งลูกค้าแค่ต้นฉบับ

ทำเป็น **ตัวเลือกตอนสั่งพิมพ์** แทนการตัดสำเนาทิ้ง เพราะสำเนากระดาษยังมีที่ใช้
โดยเฉพาะใบกำกับภาษีที่ฝ่ายบัญชีเก็บเข้าแฟ้ม:

| โหมด | ได้อะไร | ใช้เมื่อไร |
|---|---|---|
| **ต้นฉบับ** (ค่าตั้งต้น) | ต้นฉบับอย่างเดียว | ส่งลูกค้า — งานประจำวัน |
| **สำเนา** | สำเนาอย่างเดียว | พิมพ์เพิ่มเก็บแฟ้มทีหลัง โดยไม่ต้องพิมพ์ต้นฉบับซ้ำ |
| **ต้นฉบับ + สำเนา** | ทั้งสองฉบับต่อกันในไฟล์เดียว | ออกเอกสารเป็นชุด (พฤติกรรมเดิม) |

ปุ่มเลือกอยู่ใต้ตัวอย่างเอกสาร ติดกับปุ่มพิมพ์ — จุดที่ตัดสินใจจริง และสิ่งที่เห็น
บนจอคือสิ่งที่จะออกมาในไฟล์ทุกประการ

### 59.1 หมายเหตุ "(เอกสารออกเป็นชุด)" ผูกกับโหมดอัตโนมัติ

ข้อความนี้บนใบกำกับภาษีขึ้นเฉพาะโหมด "ต้นฉบับ + สำเนา" เท่านั้น เพราะต้องระบุเฉพาะ
ตอนที่ออกเอกสารเป็นชุดจริงๆ — พิมพ์ต้นฉบับใบเดียวแล้วยังขึ้นข้อความนี้คือการระบุที่ไม่ตรง
กับเอกสารที่ออกจริง เดิมผูกกับค่าคงที่ในโค้ด ตอนนี้ส่งเป็น prop `issuedAsSet`
ลงไปถึงหัวเอกสารแทน

ข้อความนี้ขึ้น**ทุกหน้าของต้นฉบับ** (ไม่ขึ้นบนสำเนา) เพราะอยู่คู่กับป้าย "ต้นฉบับ"
บนหัวเอกสารซึ่งซ้ำทุกหน้าอยู่แล้ว — พฤติกรรมเดิมไม่เปลี่ยน ถ้าฝ่ายบัญชีต้องการให้ขึ้น
เฉพาะหน้าแรก แจ้งได้

### 59.2 เลขหน้านับแยกต่อฉบับ

โหมดทั้งชุดได้ 1/2, 2/2 (ต้นฉบับ) แล้วขึ้น 1/2, 2/2 ใหม่สำหรับสำเนา ไม่ใช่ 1/4-4/4
เพราะเป็นเอกสารสองฉบับที่พิมพ์ต่อกัน ไม่ใช่เอกสารเดียว 4 หน้า

### 59.3 ไม่ต้องรัน migration

โค้ดล้วน ฐานข้อมูลยังอยู่ที่ไฟล์ 0027

ตรวจในเบราว์เซอร์จริงครบทั้งสามโหมด: จำนวนหน้า (2/2/4) ป้ายฉบับทุกหน้า เลขหน้าที่นับ
แยกต่อฉบับ และหมายเหตุ "ออกเป็นชุด" ที่ขึ้นเฉพาะใบกำกับภาษีในโหมดทั้งชุด ไม่ขึ้นบนสำเนา
และไม่ขึ้นบนใบเสนอราคาเลย

## 60. ปรับตามแคตตาล็อกบริษัท — เมนู · Dashboard · โทนสี

### 60.1 สลับลำดับเมนู — บัญชีขึ้นมาอยู่ใต้ Dashboard

ลำดับใหม่: Dashboard → **Accounting** → Project → Contact → Stock → Sourcing → Report → Team Chat
และกางกลุ่ม Accounting ไว้ตั้งแต่เปิดแอป (เดิมกาง Project) เพราะบัญชีกลายเป็นโมดูลที่ใช้บ่อยที่สุด

### 60.2 โลโก้ — ใช้ wordmark เดียวกับบนใบเสนอราคา

ตราวงกลม `ArcaSeal` บนหัวเมนูไม่ตรงกับแคตตาล็อกบริษัท เปลี่ยนเป็น `ArcaWordmark`
ซึ่งเป็นไฟล์โลโก้จริง (`public/logo-arca.png`) ตัวเดียวกับที่พิมพ์บนเอกสารทุกใบ —
โลโก้บนหน้าจอกับบนกระดาษจึงเป็นตัวเดียวกันแล้ว

ไฟล์เป็น PNG พื้นโปร่งใสสีดำ กลับสีเองไม่ได้ โหมดมืดจึงใช้ `dark:invert`

### 60.3 เมนูและกรอบหน้าจอเป็นภาษาอังกฤษ

เมนูหลักทั้งหมด หัวข้อกลุ่ม (Main / System / Support) ช่องค้นหา ทูลทิป และกล่องแจ้งเตือน
แปลเป็นอังกฤษแล้ว รวมถึงเวลาแบบสัมพัทธ์ (`5m ago`, `2h ago`)

ชื่อเอกสารในเมนูบัญชีแปลตามศัพท์บัญชีไทยที่ตรงกัน: ใบเสนอราคา = Quotation ·
ใบแจ้งหนี้ = Invoice · ใบกำกับภาษี/ใบเสร็จ = Tax Invoice / Receipt · ใบสั่งซื้อ = Purchase Order

รวมถึงเมนูรองในแถบบน: "Mark all read" ในกล่องแจ้งเตือน และ "Sign Out" ในเมนูผู้ใช้

> **ขอบเขต (ยืนยันแล้ว):** แปลเฉพาะ **เมนูหลักและเมนูรองในกรอบหน้าจอ** — เนื้อหาในหน้า
> และตัวเอกสารที่พิมพ์ยังเป็นภาษาไทย เพราะเป็นศัพท์ทางกฎหมายและบัญชีที่ต้องตรงกับเอกสารจริง
> ตรวจด้วยการสแกนโค้ดพอยต์ภาษาไทยในไฟล์ Sidebar / TopBar / AppShell แล้วไม่เหลือข้อความ
> ภาษาไทยที่ผู้ใช้มองเห็น เหลือเฉพาะคอมเมนต์ในโค้ดซึ่งคงเป็นไทยตามธรรมเนียมของโปรเจกต์

### 60.4 Dashboard ใหม่ — เรียงตามที่ใช้จริง

| ลำดับ | ส่วน | ที่มาของข้อมูล |
|---|---|---|
| 1 | **Quotations by Category** มูลค่าใบเสนอราคาแยกตามหมวดสินค้า พร้อมแถบสัดส่วน | `getQuotationsByCategory()` |
| 2 | **Outstanding Receivables** ใบที่ออกไปแล้วแต่ยังเก็บเงินไม่ครบ เรียงจากค้างนานสุด | `getUnpaidBills()` |
| 3 | **Top 5 Stock by Quantity** ของในคลังมากที่สุด 5 อันดับ | `getTopStockItems()` |
| 4 | Overdue / Needs Action (เดิม ย้ายลงมา) | `getOverdueItems()` |
| 5 | Project & Ticket Pipeline — เนื้อหาเดิม **ย่อขนาดลง** เป็นแถบนับ 4 ช่อง | เดิม |
| 6 | PM Requests Pending — ย่อเหลือแถบนับ รายการละเอียดอยู่ในหน้าของมันเอง | เดิม |

ฟังก์ชันใหม่ทั้งสามอ่านอย่างเดียว **ไม่แตะสูตรคำนวณของโมดูลบัญชี** — ใช้ `grand_total`
และ `paid_amount` ที่ `saveArDocument` คำนวณและแช่ไว้ในเอกสารแล้ว

จุดที่ตัดสินใจไว้:
- ใบเสนอราคาที่ **ยกเลิกแล้วไม่นับ** เพราะไม่ใช่ข้อเสนอที่ยังอยู่บนโต๊ะ
- ใบที่ **ไม่ได้ติด Tag** รวมไว้ใต้ "ไม่ระบุหมวด" ไม่ตัดทิ้งเงียบ ยอดรวมบน Dashboard
  จึงตรงกับยอดรวมในหน้าใบเสนอราคาเสมอ
- **ค้างชำระ** = `grand_total − paid_amount` ของใบที่ออกเลขแล้วและยังไม่ยกเลิก
  ใบที่เหลือค้างไม่ถึงสลึงถือว่าเก็บครบ · "เลยกำหนด" นับเฉพาะใบที่ตั้งวันครบกำหนดไว้
- **สต็อก** รวม `on_hand` ข้ามทุกคลังเฉพาะ pool `normal` — ของชำรุด ของที่ถูกยืมไป
  และของที่หายไม่ใช่ของที่หยิบมาขายได้ จึงไม่นับในอันดับนี้

### 60.5 โทนสีตามแคตตาล็อก — จาก slate เป็น stone

ทั้งแอปเปลี่ยนจากเทาอมฟ้า (`slate`) เป็นเทาอุ่น (`stone`) ทั้งหมด 2,068 จุดใน 43 ไฟล์
ให้ตรงกับแคตตาล็อกซึ่งใช้ขาว/เบจ/เขียวมะกอก/น้ำตาลเข้ม ไม่มีโทนฟ้าเลย
ค่าความสว่างของทุกเฉดเท่าเดิม เปลี่ยนแค่เนื้อสี คอนทราสต์จึงไม่เพี้ยน

เพิ่ม token `--brand-sand` และ `--brand-olive` จากสีจริงของหน้ากากสวิตช์ Modern Series
ไว้ใช้เป็นสีเน้นในอนาคต

### 60.6 ไม่ต้องรัน migration

โค้ดล้วน ฐานข้อมูลยังอยู่ที่ไฟล์ 0027

ตรวจในเบราว์เซอร์จริง: เมนูเรนเดอร์ครบเป็นอังกฤษพร้อม wordmark ถูกต้อง · Dashboard
เรนเดอร์ครบทั้ง 6 ส่วนตามลำดับที่กำหนดโดยไม่มี error แม้ทุก query จะล้มเหลว
(สถานะว่าง/ผิดพลาดปลอดภัย ไม่ทำหน้าจอพัง)

## 61. ชื่อไฟล์ PDF ตั้งให้อัตโนมัติ

กด "พิมพ์ / บันทึกเป็น PDF" แล้วช่อง File name เติมมาให้เลยเป็น
`เลขที่เอกสาร - ชื่องาน` เช่น `QT202609008 - Resort At Rayong` ไม่ต้องพิมพ์เอง

เบราว์เซอร์เอา `document.title` มาเป็นชื่อไฟล์ตั้งต้นของ Save as PDF ซึ่งเดิมเป็นชื่อแอป
เหมือนกันทุกใบ จึงตั้งค่าให้ก่อนเรียก `window.print()` แล้วคืนค่าเดิมเมื่อปิดหน้าต่างพิมพ์
(ฟัง `afterprint` และมี timeout กันเหนียวเผื่อเบราว์เซอร์ไม่ยิง event) ไม่งั้นชื่อแท็บ
จะค้างเป็นเลขที่เอกสารไปจนกว่าจะรีเฟรช

### 61.1 ล้างอักขระที่ตั้งเป็นชื่อไฟล์ไม่ได้

ชื่องานจริงในระบบมีเครื่องหมายคำพูดอยู่บ่อย เช่น `Door lock Sale Gallery "Love it" Project`
ถ้าปล่อยไปตรงๆ Windows จะไม่ยอมบันทึกหรือตัดชื่อทิ้งเอง จึงแทน `\ / : * ? " < > |`
และอักขระควบคุม (ขึ้นบรรทัดใหม่ แท็บ) ด้วยช่องว่าง แล้วยุบช่องว่างซ้ำ — วงเล็บและขีดกลาง
เก็บไว้ได้ ชื่ออย่าง `Asia Hotel Bangkok (Option-Hyundai)` จึงออกมาครบ

### 61.2 กรณีข้อมูลไม่ครบ และโหมดพิมพ์

- ร่างที่ยังไม่มีเลขที่ → ใช้ชื่องานอย่างเดียว · ยังไม่ได้ตั้งชื่องาน → ใช้เลขที่อย่างเดียว
- ไม่มีทั้งคู่ → `document` (กันชื่อไฟล์ว่าง)
- โหมด **สำเนา** ต่อท้าย ` (สำเนา)` และโหมดทั้งชุดต่อท้าย ` (ต้นฉบับ+สำเนา)` —
  พิมพ์สำเนาแยกทีหลังจึงไม่เขียนทับไฟล์ต้นฉบับที่บันทึกไว้แล้ว
- ตัดความยาวก่อนต่อวงเล็บเสมอ คำว่า (สำเนา) จึงไม่ถูกตัดหายเมื่อชื่องานยาวมาก

ไม่ต้องรัน migration — โค้ดล้วน ฐานข้อมูลยังอยู่ที่ไฟล์ 0027

## 62. หน้า Login ใช้โลโก้ ARCA เหมือนที่อื่น

หน้าเข้าสู่ระบบยังเป็นตราวงกลม `ArcaSeal` + ข้อความ "ARCA HAUS" อยู่ที่เดียว
เปลี่ยนเป็น `ArcaWordmark` (ไฟล์ `public/logo-arca.png`) ให้ตรงกับเมนูและเอกสารที่พิมพ์
ตอนนี้โลโก้ทั้งระบบมาจากไฟล์เดียวกันหมดแล้ว

โหมดมืดใช้ `dark:invert` เพราะไฟล์เป็น PNG สีดำพื้นโปร่งใส กลับสีเองไม่ได้ —
ตรวจด้วยการวัดค่าความสว่างพิกเซลจริงทั้งสองโหมด: สว่าง L=0 บนพื้น L=255 · มืด L=255
บนพื้น L=10 คอนทราสต์เต็มทั้งคู่

### 62.1 สองอย่างที่จงใจไม่เปลี่ยน

- **`ArcaSeal.tsx` ไม่ถูกลบ** แม้ตอนนี้ไม่มีที่ไหนเรียกใช้แล้ว เพราะตราวงกลมเป็นสินทรัพย์
  แบรนด์จริงที่อาจต้องใช้กับตราประทับเอกสารภายหลัง (ดู §55 เรื่องตราประทับบริษัท)
- **favicon ยังเป็น `seal.svg`** เพราะที่ขนาด 16px คำว่า ARCA จะอ่านไม่ออก
  ตราวงกลมทำงานได้ดีกว่ามากในขนาดนั้น
